...
class anomaly_experiment:
    def __init__(self, connection, graph, batch_size, instances, preprocessing, train_flow_anomalies=0.0,
                 ...
                 only_decision=False):
        self.only_decision = only_decision
    ...
    def run(self):
        ...
        if only_first_anomaly:
            label_anomalies = 'only_first_grid'
        else:
            label_anomalies = 'all_grid'
        if self.only_decision:
            label_anomalies += '_decision'
        ...
        fig.savefig(get_assets_path() / 'Scatterplots' / 'AnomalyDetectionDecision' / name, format='pdf')
        plt.show()

        for threshold in [x * 0.01 for x in range(0, 15)]:
            predicted_anomalies = [error > threshold for error in mse]
