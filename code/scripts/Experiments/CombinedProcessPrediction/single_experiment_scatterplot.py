...
def plot_scatter(connection, instances, only_decision=False, flow_anomalies=0.0,
    ...
    for index, row in df.iterrows():
        advanced = ['norm_rnn', 'norm_cnn', 'kalman_rnn', 'kalman_cnn', 'kal_norm_rnn', 'kal_norm_cnn', 'baseline_id',
                    'baseline_attr', 'baseline_sensor']
    ...
    ax.set_xlim([0, 1])
    fig.tight_layout()
    fig.savefig(get_assets_path() / 'Scatterplots' / 'CombinedProcessPrediction' / name, format='pgf')
    plt.show()
    ...
    if save_legend:
        ...
        ax_leg.axis('off')
        fig_leg.savefig(get_assets_path() / 'Scatterplots' / 'CombinedProcessPrediction' / 'legend.pgf', format='pgf')
