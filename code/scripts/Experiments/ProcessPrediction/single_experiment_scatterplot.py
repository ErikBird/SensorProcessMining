...
def plot_scatter(connection, instances, only_decision=False, flow_anomalies=0.0,
    ...
    for index, row in df.iterrows():
        ...
        if row['preprocessing'] not in advanced:
            marker = "x"
    ...
    ax.set_xlim([0, 1])
    fig.tight_layout()
    fig.savefig(get_assets_path() / 'Scatterplots' / 'ProcessPrediction' / name, format='pgf')
    plt.show()
    ...
    if save_legend:
        ...
        ax_leg.axis('off')
        fig_leg.savefig(get_assets_path() / 'Scatterplots' / 'ProcessPrediction' / 'legend.pgf', format='pgf')
